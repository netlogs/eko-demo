...
import { Tool, InputSchema, ExecutionContext } from "@eko-ai/eko/types";
...
class DirectoryFormatter implements Tool<any, any> {
  ...
  async execute(context: ExecutionContext, params: any) {
    const { entries, format = "simple" } = params;

    if (format === "simple") {
      return entries.map((entry: any) => entry.name).join("\n");
    }

    return entries
      .map(
        (entry: any) => `${entry.name} - ${entry.size} bytes - ${entry.modifiedAt}`
      )
      .join("\n");
  }
}
